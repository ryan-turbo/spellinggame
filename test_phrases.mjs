...
// 测试词组
const tests = [
  ['have a shower', '/hæv ə ˈʃaʊə/'],
  ['listen to music', '/ˈlɪsən tə ˈmjuːzɪk/'],
  ['go skating', '/ɡəʊ ˈskeɪtɪŋ/'],
  ['get up', '/ɡet ʌp/'],
...
for(const [w,p] of tests){
  const r = s(w,p);
  console.log(w.padEnd(20)+' => syllables: '+r.length);
  r.forEach((s,i) => console.log('  ['+i+'] letters="'+s.l+'" ipa='+s.i));
  console.log('  total letters: '+r.reduce((a,s)=>a+s.l.length,0)+' (expected: '+w.replace(/ /g,'').length+')');
}
