# -*- coding: utf-8 -*-
"""Diagnose IPA syllable/letter mismatch - using correct UTF-8 IPA"""
import re, sys, io
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')

# Current App.jsx vowel list - using real Unicode IPA characters
VOWEL_PHONEMES = [
...
def split_syllables(word, phonetic):
    ...
    for i in range(1, len(vowels)):
        ...
        gap_clean = gap.replace('ˈ','').replace('ˌ','')
    ...
    ipa_lengths = [len(ip.replace('ˈ','').replace('ˌ','')) for ip in syllable_ipas]
...
for name, filepath in [('PU2', 'web/src/data/pu2_vocab.js'), ('PU3', 'web/src/data/pu3_vocab.js')]:
    ...
    for w in words:
        ...
        word_clean = word.replace(' ', '')
        ipa_clean = stripped.replace('ˈ','').replace('ˌ','')
        ...
        if num_ipa_vowels != num_syllables:
            issues.append(f"IPA_vowel({num_ipa_vowels}) != syllable({num_syllables})")

        # Rule 2: Overall letter/IPA ratio - should be 0.85-1.2
        if total_ipa > 0:
            ratio = total_letters / total_ipa
            if ratio < 0.85 or ratio > 1.25:
                issues.append(f"RATIO: {total_letters}L/{total_ipa}I={ratio:.2f}")

        # Rule 3: Per-syllable suspicious ratio (too high = too many letters per IPA char)
        for i, syl in enumerate(syllables):
            syl_ipa = syl['ipa'].replace('/','').replace('ˈ','').replace('ˌ','')
        ...
        if issues:
            ...
            print()
            unit_problems.append({'word': word, 'phonetic': phonetic, 'syllables': syllables, 'issues': issues})

    all_problems.extend(unit_problems)
    print(f"  {name} problems: {len(unit_problems)}")
