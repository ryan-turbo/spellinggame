...
function s(w,p){
  ...
  for(let ii=1;ii<vu.length;ii++){
    ...
    else{si.push(st.slice(prev,gs+1));prev=gs+1;}
  }
  ...
  function aL(letters,syllableIpas){
    ...
    for(let sii=0;sii<syllableIpas.length;sii++){
      ...
      if(sii===si.length-1&&lPos<letters.length){syll+=letters.slice(lPos);lPos=letters.length;}
      result.push(syll);
    }
    return result;
  }
...
const tests=[
  ['Wednesday','/ˈwenzdeɪ/'],['library','/ˈlaɪbrəri/'],['strawberry','/ˈstrɔːbəri/'],
  ['yoghurt','/ˈjɒɡət/'],['headache','/ˈhedeɪk/'],['listen to music','/ˈlɪsən tə ˈmjuːzɪk/'],
  ['go skating','/ɡəʊ ˈskeɪtɪŋ/'],['university','/ˌjuːnɪˈvɜːsəti/'],
];
let pass=0,fail=0;
for(const[w,p]of tests){
  const r=s(w,p);
  const got=r.map(x=>x.l).join('').toLowerCase();
  const exp=w.replace(/ /g,'').toLowerCase();
  const ok=got===exp;
  if(ok)pass++;else fail++;
  console.log((ok?'✓':'✗')+' '+w.padEnd(20)+' => '+r.map(x=>'['+x.l+']').join(' '));
